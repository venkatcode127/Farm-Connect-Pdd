...
public class JsonReportGenerator {
    ...
    public static void generateJsonReport(List<Map<String, String>> results, String filePath) {
        try {
            ...
            summary.addProperty("passRate", total > 0 ? Math.round(passed * 10000.0 / total) / 100.0 : 0);
        ...
        try {
            ...
            for (Map<String, String> tc : results) {
                if ("PASS".equals(tc.get("status"))) {
                    md.append("- ✓ ").append(tc.getOrDefault("testId", "")).append(" - ").append(tc.getOrDefault("testName", "")).append("\n");
                }
            }
