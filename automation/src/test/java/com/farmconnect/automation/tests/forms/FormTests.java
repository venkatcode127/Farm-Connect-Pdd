...
public class FormTests extends BaseTest {
    ...
    @Test(priority = 2, retryAnalyzer = RetryAnalyzer.class) public void TC_FORM_006_selectDropdownHasOptions() { navigateToSection("prediction"); int opts = (int)(long)(Long)executeScript("var s=document.querySelector('#prediction select');return s?s.options.length:0;"); Assert.assertTrue(opts >= 1, "Dropdown should have options"); }
    ...
    @Test(priority = 3, retryAnalyzer = RetryAnalyzer.class) public void TC_FORM_010_formLabelsPresent() { navigateToSection("prediction"); int labels = (int)(long)(Long)executeScript("return document.querySelectorAll('#prediction label').length;"); Assert.assertTrue(labels >= 0); }
    ...
    @Test(priority = 3, retryAnalyzer = RetryAnalyzer.class) public void TC_FORM_020_formAccessibleLabels() { clearAppState(); int labels = (int)(long)(Long)executeScript("return document.querySelectorAll('#authScreen label').length;"); Assert.assertTrue(labels >= 2); }
}
